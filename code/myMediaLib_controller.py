...
class MediaLib_Controller(MediaLibPlayProcess_singletone_Wrapper):
	def __init__(self):
		...
		# âûçûâàåì ÿâíî êîíñòðóêòîð ðîäèòåëÿ, ÷òîáû â ýòîì êëàññå èìåòü äîñòóï ê àòðèáóòàì ðîäèòåëÿ
		# òàêèì îáðàçîì èíñòàíöèÿ êîíòðîëåðà ÿâëÿåòñÿ îäíîâðåìåííî èíñòàíöèåé ìîäåëè, çà÷åì ýòî???? 
		# ýòî ñäåëàíî íà íà÷àëüíîì ýòàïå ÷òîáû ìèíèìàëüíî èçìåíÿòü ñòàðûé êîä, êîãäà â ìîäåëè áûë ðåàëèçîâàíè êîíòðîëëåð 
		# íàïðÿìóþ. â áóäóùåì ñäåëàòü â ìîäåëè ìåòîäû, êîòîðûå âûäàâàëè áû ñîîòâåòñâóùèå îáúåêòû ìîäåëè 
		#super(MediaLib_Controller, self).__init__()
		
		# Îäíîé èç íåîáõîäèìûõ çàäà÷ êîíòðîëëåðà MVC äîëæíà áûòü ïîîäåðæêà ìîäåëè â àêòóàëüíîì ñîñòîÿíèè
		# ýòè ôóíêöèè íåáîõîäèìî ïåðåíåñòè ñþäà èç ñòàðîé ôóêöèè ãåíåðàöèè ñòðàíèö
		#global logger
		...
		# Ïîëó÷èòü ñïèñîê ñèãíàòóð êëàññà ãåíåðàòîðà ïðåäñòàâëåíèÿ MediaLib_ViewGen
		...
		# â ýòîì ñëîâàðå ñîäåðæàòüñÿ ñâÿçè:
		#		èäåíòèôèêàòîðà îòðàáîòàâøåãî ýëåìåíòà óïðàâëåíèÿ:
						#'player_control'- áàçîâûå ôóíêöèè ïëååðà
						# "navigation_control" - çàãðóçêà íîâûõ ëèñòîâ è íàâèãàöèÿ ïî ëèñòàì ñ íåîáõîäèìîé ðåàêöèåé ïëååðà(ëèñòû, òýãè)
		# 		ìåòîäà èçìåíåíèÿ ñîñòîÿíèÿ ìóç ïëååðà è  ìîäåëè îòðàæàþùåé åãî ñîñòîÿíèå. 
		#		ñïèñêà ýëåìåíòîâ ïðåäñòàâëåíèÿ òðåáóþùèõ îáíîâëåíèÿ, ñïè
	...
	def get_modelDic(self,view_elem_id_Dic,PlayControl_CurStatusD):
		# Èíòåðôåñ ê ìîäåëè: ìåòîä ïðèìåíÿåòñÿ äëÿ òîãî ÷òîáû ïîëó÷èòü èç ìîäåëè äàííûå õàðàêòåðèçóþùèå ñîñòîÿíèå ïëååðà è ïðèëîæåíèÿ
		# òóò íå ïðåäïîëîãàåòñÿ íèêàêèõ âû÷èñëåíèé, òîëüêî ïîëó÷åíèå óæå ãîòîâûé ñòðóêòóð
		# èçâëå÷åíèå è âû÷èñëåíèå ïðîèñõîäÿò íàïðèìåð â ôóíêöèè get_data_for_player_context
		...
		if 'tag_name' in view_elem_id_Dic:
			...
			try:
				# â äàííîì ñëó÷àå îæèäàåòñÿ ÷òî ïåðåìåííàÿ PlayControl_CurStatusD èìååò çíà÷åíèå âîçâðàùåííîå èç ìåòîäà êîíòðîëëåðà int
		...
		if 'search_res_buf' in view_elem_id_Dic:
			...
			# Ñãåíåðèðóåì ñëîâàðü òýãîâ tagsLDic äëÿ ñâÿçàííûõ ñ íèìè êîìïîçèöèé. Ãåíåðèðóåì äèíàìè÷åñêè, òåîðåòè÷åñêè ýòî ìîæíî ïåðåíåñòè â êîíòðóêòîð.
			tagsLDic =  {}
			DB_metaIndxD = self.__model_instance.MediaLibPlayProcessDic_viaKey('DB_metaIndxD','local')	
			
			# Ãåíåðèðóåì ñëîâàòü ñîîòâåòñòâèé èíäåêñîâ òðýêîâ â ÁÄ è èõ ÑÐÖ32	
		...
		if 'albumD'	 in view_elem_id_Dic:
			# Íóæåí âðîäå äëÿ âûäà÷è êàðòèíîê, è ôîðìèðîâàíèÿ äðîïäàóí ëèñòà àëüáîìîâ
		...
		if 	'artist_search_vars' in view_elem_id_Dic:	
			modelDic['Tmpl'] = self.__model_instance.MediaLibPlayProcessDic_viaKey('Tmpl','local')
			# â äàííîì ñëó÷àå îæèäàåòñÿ ÷òî ïåðåìåííàÿ PlayControl_CurStatusD èìååò çíà÷åíèå âîçâðàùåííîå èç ìåòîäà êîíòðîëëåðà [list]
		...
		if 'cast_frame3' in view_elem_id_Dic:
			# Ïîëó÷àåì  ñîäåðæèìîå ëèñòà îò êîíòðîëëåðà ïëååðà
			print 'cast_frame3 -1'
			modelDic['Tmpl'] = self.__model_instance.MediaLibPlayProcessDic_viaKey('Tmpl','local')
			print 'cast_frame3 -2'
	...
	def command_dispatcher(self,signatura_pool,*attr):
		...
		http_Reply = ''
		self.__commandRoutingDic = self.__model_instance.MediaLibPlayProcessDic_viaKey('commandRouting','local')
		# Íèæå â öèêëå îïðåäåëÿåì ìåòîäû ïðåäñòàâëåíèÿ è ìåòîäû êîíòðîëëåðà ñîòâåòñòâóþùèå çàïðîñó ÷åðåç ñõåìó self.__commandRoutingDic
		...
		#print '-->1',a,signatura
		# Òàê êàê äèñïåò÷åð óíèâåðñàëüíûé òî äàííûå ïðîâåðêè ðåëåâàíòíû òîëüêî äëÿ ïðîöåññà êîíòðîëÿ ïëååðà
		if 'logic_process' in self.__commandRoutingDic[a][command]:
			if 'player_state' in self.__commandRoutingDic[a][command]['logic_process']:
				...
				#ñîõðàíèòü òåêóùåå ñîñòîÿíèå ìîäåëè
				old_stateDic = self.__model_instance.getMediaLibPlayProcess_State()
				#print 'old_stateDic song key:',old_stateDic['PlayerControl']
		
		# Âûçâàòü íåîáõîäèìûé ìåòîä êîíòðîëëåðà, - ìîäèôèêàöèÿ ìîäåëè â ñîîòâåòñòâèè ñ äàííîé êîìàíäîé è òåê. ñîñòîÿíèåì ïëååðà
		# â çàâèñèìîñòè îò çàïðîñà âûçûâàåì ìåòîä ñ ïàðàìåòðàìè èëè áåç
		...
		if 'logic_process' in self.__commandRoutingDic[a][command]:
			...
			if 'player_state' in self.__commandRoutingDic[a][command]['logic_process']:
				...
				# Ïðîâåðèòü ïðîèçîøëî ëè èçìåíåíèå òåêóùåãî àëüáîìà åñëè äà òî â ãåíåðàöèè äëÿ ãåíåðàöèè âüþ çàïðîñèòü íîâûé ñïèñîê òðåêîâ
				cur_stateDic = self.__model_instance.getMediaLibPlayProcess_State()
		
				# Îïðåäåëÿåì íóæíî ëè ïåðåñëàòü êàðòèíêó àëüáîìà
				if 'cover' in view_elemD:
					try:
				...
				# Áëîê ñèíõðîíèçàöèè äàííûõ ïëåéëèñòà, åãî ñîäåðæèìîãî, åãî êàê ñðö32L ïðåäñòàâëåíèÿ, è ìåòà äàííûõ ñ íèì ñâÿçàííûõ
				# àíàëîãî ôóíêöèè client_layout_merge
					try:	
		...
		# Ïåðåä âûçîâîâ ìåòîäà view MVC íåîáõîäèìî ïîäãîòîâèòü âñå óïðàâëÿþùèå ñòðóêòóðû, ñëîâàðè, ñïèñêè ýòè ýëåìåíòû ìû áåðåì èç ìîäåëè MVC
		# profile  this -------------------------------
		...
		#----------------------
		# Òóò ïðîèñõîäèò âûçîâ ìåòîäà ïðåäñòàâëåíèÿ, âîçâðàùàþùåãî ëèáî ãîòîâóþ ñòðàíèöó ëèáî json îòâåò äëÿ ÷àñòè ñòðàíèöû
		# self.__instance_VIEW - èíñòàíöèÿ êëàññà MediaLib_ViewGen (medialib_pages.py)
		# signatura_view_method - ìåòîä êëàññà MediaLib_ViewGen àâòîìàòè÷åñêè îïðåäåëåííûé ÷åðåç ñõåìó  self.__commandRoutingDic
		#-----------------------
	...
	# ôóíêöèÿ - ðàñøèðåíèå äëÿ get_modelDic, ñëîæíûå àëãîðèòìû â ðàìêàõ get_modelDic äîëæíû ðåàëèçîâàûâàòüñÿ â ïîäîáíûõ ôóíêöèÿõ.
	# ôóíêöèÿ ïî çàäàíèÿì îïðåäåëåííûì â view_elemD è àíàëèçèðóÿ òåêóùèé áóôåð äàííûõ modelDic
	# ñîáèðàåò äàííûå êîíòåêñòà ïëååðà è ïîìåùàåò èõ ïî êëþ÷àì â modelDic
		...
		# Çàìåíèòü íèæíþþ êîíñòðóêöèþ íà âçÿòèå äàííûõ èç ìîäåëè.
		...
		#òóò âû÷èñëèòü òèï àëüáîìà íà ÍÑÀ è ñîõðàíèòü ýòîò àòðèáóò isNSA
		isNSA = False
	...
	# ÂÍÈÌÀÍÈÅ!!! Íè â êîåì ñëó÷àå íå îáðåçàòü äëèíó íàçâàíèÿ òðýêà, àëüáîìà, àðòèñòà ýòî ïðèâîäè ê îøèáêè äåêîäèðîâàíèÿ UTF			
	...
	# ÂÍÈÌÀÍÈÅ!!! Íè â êîåì ñëó÷àå íå îáðåçàòü äëèíó íàçâàíèÿ òðýêà, àëüáîìà, àðòèñòà ýòî ïðèâîäè ê îøèáêè äåêîäèðîâàíèÿ UTF
			...
			if 'album_initial' in args:
			# òóò ïðîâåðèòü ñîâïàäàåò ëè êîëè÷åñòâî òðýêîâ â ëèñòå ñ êîëè÷åñòâîì ðåàëüíûõ òðýêîâ â àëüáîìå, è åñëè íåò, òî ñäåëàòü ñïèñîê âîñòàíîâëåííûõ òðýêîâ
	...
	def client_layout_merge(self,mode,client_state,pc, view_elemD):	
		# Îïðåäåëÿåò íåîáõîäèìûå èçìåíåíèÿ â ïðåäñòàâëåíèè êëèåíòà íà îñíîâå åãî òåêóùåãî ñòàòóñà!!client_state!!
		...
		if client_state['track_CRC32'] <> pc['track_CRC32']:		
			...
			view_elemD['tagsL']=''
			return	
		# íå áóäåò èçìåíåíèé íî íàäî âîññòàíîâèòü óáèòûå â view ïåðåìåííûå	
	...
	def refresh_check_player_state(self,commandD):
		...
		if pc['playBack_Mode']	== 0 and MediaLibPlayProcess_StateD['manual_stop_flag'] == False and pc['list_length'] == (MediaLibPlayProcess_StateD['songNum']+1) and lenQueue>1: 
			...
			# Ãåíåðèðóåì åãî ñîäåðæèìîå êàê crc32L
			#try:
		...
		# ---> 	ÂÍÈÌÀÍÈÅ - Íåîáõîäèìà àêòóàëèçàöèÿ ìîäåëè
	...
	def forward(self):
		...
		# Ïðè ïåðåìîòêàõ íå òðåáóåòñÿ èçìåíåíèå ìîäåëè, òàê êàê åäèíñòâåííî, ÷òî ìåíÿåòñÿ ïðè ýòîì ýòî "âðåìÿ äî êîíöà",
		# à îíî âîçâðàùàåòñÿ â ñòàòóñå è ïåðåäàåòñÿ â ãåíåðàòîð ïðåäñòàâëåíèÿ íàïðÿìóþ
...
#	Ýòî íàäî ìîäèôèöèðîâàòü ïîä íîâóþ àðõèòåêòóðó	
	...
	def goto_tag_general(self,tagId,tagType,title_crc32):
		...
		if title_crc32 == None:
			# Åñëè ïåðåãåíåðàöèÿ íà îñíîâå äàííûõ òåêóùåé êîìïîçèöèè, òî ñîõðàíÿåì åå ÑÐÖ32
			PlayControl_CurStatusD = self.__player_handler.get_status()
			curCRC32 = PlayControl_CurStatusD['track_CRC32']	
		elif title_crc32 == 0:
			# Åñëè íåò, òî ïåðåõîä â òî÷êó óêàçàííóþ â ïàðàìåòðàõ 
		...
		# Ïðîâåðêà êîíöåïöèè çàãðóçêè ëèñòîâ ìèíóÿ ôàéëîâóþ ñèñòåìó	
		t_start = time.time()
		if tagType == 'hard_tag':	
	...
	def cast_goto_Track(self,commandD):	
		...
		# Ñîçäàåì â ðàìêàõ äóììè ñåññèè äàííûå ïî åå òåêóùåìó ëèñòó â áóäóùåì ñåññèÿ ñîçäàåòñÿ ïðè ëîãèíå è òóò âìåñòî 1 ïåðåäàåòñÿ íàñòîÿùèé èíäåêñ ñåññèè	
	...
	def play_rel_lib_pl(self,commandD):	
		...
		mediaPath =	self.__model_instance.MediaLibPlayProcessDic_viaKey('configDict','local')['mediaPath']
		list_path_name = mediaPath+"Plugins\\ml\\"+listKey
		# òàê êàê ïîêà ïîçèöèÿ â ëèñòå íåèçâåñòíà, òî ñòàâèì åå íà íà÷àëî ñïèñêà
		pc = self.__player_handler.new_list_load(list_path_name,track_pos)
		
		# Ïîëó÷àåì  ñîäåðæèìîå âñåõ CRC32 çíà÷åíèé ëèñòà îò êîíòðîëëåðà ïëååðà
		PlayList_asCRC32_L = self.__player_handler.get_cur_pl_as_list_of_crc32()
		
		# Ïîëó÷àåì íîìåð ïîçèöèè â ñïèñêå
	...
	def play_search_selectedL_new(self,commandD):	
		# Íîâûé ìåòîä çàãðóçêè ïëåéëèñòà ÷åðåç ñïèñîê CRC32 òðýêîâ áåç èñïîëüçîâàíèå áóôåðà ïðèäûäó÷åãî ïîèñêà
	...
	def play_search_selectedL(self,commandD):
		...
		# Óáðàòü íèæå ñòîÿùèé áóôåð è áðàòü èç ÁÄ ïî db_id
		sD = searchBuf['sD']
		copy_sD = {}
	...
	def	do_artist_search(self,commandD):	
		# Â ýòîé ôóíêöèè â çàâèñèìîñòè îò êîìàíäû èñêàòü ëèáî â áóôåðå âñåõ ìåòàäàííûõ ëèáî òîëüêî â ìåòàäàííûõ ïî áàçå Àðòèñòîâ.
		...
		if commandD['artists_maintain'] == "search_artist_db":
			...
			# ÂÍÈÌÀÍÈÅ! proc_state - ÿâëÿåòñÿ êëþ÷åì äëÿ bufferD													
		...
		# Åñëè ìåòàäàííûå åùå íå ïîëó÷åíû òî íèæåñòîÿùàÿ ôóêöèÿ èõ ñîáåðåò, åñëè óæå áûëè ñîáðàíû,òî èç íåå áûñòðûé âûõîä
			...
			if 'ALBLOG' in idl_param and folder_keyL <> []:
				...
				# Ïîëó÷àåì òåêóùèå ìåòàäàííûå ARTIST --> ALBUMES íà îñíîâå äàííûõ èç òðýêîâ
				All_metaD = self.__model_instance.get_All_metaD()['resD']
				
				folder_filter_key = self.__model_instance.get_All_metaD_filter_key()
				# Â áóôôåðå êîíòðîëëåðà All_metaD íàõîäèòñÿ òîëüêî äëÿ îòëàäî÷íûõ öåëåé, åãî íàäî óäàëèòü
			...
			# Íàäî ñäåëàòü òàêîé æå ñïèñîê òîëüêî ñî ñòîðîíû àëüáîìîâ íî íå âñåõ à òîëüêî ñáîðíèêîâ ïî êðèòåðèþ ïåñíè ðàçíûõ àðòèñòîâ
			#print artistD.keys()
		...
		# ýòà ôóíêöèÿ î÷åíü ìåäëåííàÿ òàê êàê ñîáèðàåò ôàêòè÷åñêè âñþ ñòàòèñòèêó ÀÐÒÈÑÒ->ÀËÜÁÎÌ->Ñïèñîê òðýêîâ ID è Ñòàòèñòèêà òðýêîâ è àëüáîìîâ 
			self.__model_instance.set_All_metaD(srch_folder_key)
			self.__logger.debug('before get_All_metaD')
			# Ïîëó÷àåì òåêóùèå ìåòàäàííûå ARTIST --> ALBUMES íà îñíîâå äàííûõ èç òðýêîâ
			All_metaD = self.__model_instance.get_All_metaD()['resD']
			...
			# òóò áåðåì ìåòàäàííûõ äàííûå èç òàáëèöû Àðòèñòîâ, åñòåñòâåííî áåç æèâîé ñòàòèñòèêè àëüáîìîâ è òðýêîâ
			# ôèëüòðóåì ïî ïîèñêîâîìó ñëîâó 
			# óñòàíàâëèâàåì âñå áóôôåðû
			
			pass
	
	def map_artist_album_categ_assignment_to_view_json(self,artist_album_categ_structD,process_phase):
		# Íàçíà÷åíèå ôóíêöèè ïîäãîòîâèòü âüþ ïî àíàëîãèè ñ âüþ íà÷àëüíîé çàãðóçêè èëè ÄÁ âüþ
		# Äàííûå îá èíäåêñàõ àðòèñòîâ è àëüáîìîâ ïîëó÷àþòñÿ èç ÷òåíèÿ íàçíà÷åíèÿ êàòåãîðèçàöèé
		# 
		# Ò.ê àëüáîìû èíôîðìàöèè î ïåðåñå÷åíèè àðòèñòîâ è àëüáîìîâ â ñàòåãîðèçàöèîííîì íàçíà÷åíèè íåò 
		# òî åãî íàäî âîññòàíîâèòü
		# âî View ïîêàçûâàþòñÿ òîëüêî òå îáúåêòû, êîòîðûå ÿâíî ïðèñóòñòâóþò â êàòåãîðèçàöèè
		...
		# Âîññòàíàâèâàåì ïåðåñå÷åíèå àðòèñòîâ è àëüáîìîâ
		...
		# ×èòàåì ìåòàäàííûå ïî àëüáîìàì
		tb_removed_NSA_L = []
		for albumcrc32 in albumL:
			# Ñóòü ýòîé âåòêè ïîëó÷èòü ìåòàäàííûå àëüáîìà è âîññòàíîâèòü àðòèñòà ïî àëüáîìó åñëè îí íå íàçíà÷åí â êàòåãîðèè, åñëè àîëüáîì NSA òî çàñóíóòü åãî â ÍÑÀ ëèñò
		...
		# Êëþ÷è {'key':a[0],'value':a[1],'album_type':a[2]} 
	...
	def map_db_artist_album_struct_to_view_json(self,db_artist_album_structD,process_phase):
		...
		for a in artistD:
			obj_type = 'artist'	
			# åñëè àðòèñò äëÿ NSA ïðîïóñêàåì åãî òóò
	...
	def map_meta_album_tracks_struct_to_view_json(self,meta_tracks_structD,album_artistD):
		# Ôîðìèðîâàíèå ñòðóêòóðíîãî ïðåäñòàâëåíèÿ äëÿ JSON óïàêîâêè è ïåðåäà÷è íà web êëèåíò
	...
	def map_meta_artist_album_struct_to_view_json(self,meta_artist_album_structD,process_phase):
		# ôîðìèðîóåò íà÷àëüíûå áóôåðà äëÿ äâîéíîãî ïðåäñòàâëåíèÿ 'artistD_list':artistD_list,'not_single_artist_albumD_list':not_single_artist_albumD_list
		# ÂÍÈÌÀÍÈÅ album_song_num êàñàåòñÿ òîëüêî ïðåäñòàâëåíèÿ àëüáîìà ïî àðòèñòó NSA â ïðåäñòàâëåíèè àðòèñòà åñòü song_num ò.å. êîëè÷åñòâî ïåñåí ïî ýòîìó
		# àðòèñòó â àëüáîìå	
		...
		for a in ReportBufD['statL']:
			...
			for b in ReportBufD['artistD'][a[3]]:
				...
				if b in album_KeyL:
						...
						# ñäåëàòü ïåðâóþ ïðîâåðêó íà íàëè÷èå â äá è åñëè òîëüêî íîâûå òî äàëåå äîïðîâåðèòü è åñëè îïÿò â ÄÁ òî ïðîïóñê.
					if b == 'albumD':
		...
		# Ïðîâåðÿåì åñòü ëè àëüáîìû â êîòîðûõ çàäåéñòâîâàí íå òîëüêî âûÿâëåíûûå âûøå àðòèñòû
		# Äëÿ òàêèõ àëüáîìîâ äåëàåì ñïåöèàëüíóþ ñòðóêòóðó äëÿ êîòîðîé áóäåò äîïîëíèòåëüíîå ïðåäñòàâëåíèå		
		not_single_artist_albumD_list = []
	...
	def client_struct_restore(self,client_struct,server_struct,list_node_key,new_process_type):
		# ôóíêöèÿ ïðîâåðÿåò ëîãè÷åñêóþ öåëîñòíîñòü àðòèñòîâ è èõ àëüáîìîâ è ñèõðîíèçèðóåò êëèåíòñêóþ è ñåðâåðíóþ ñòðóêòóðû â ïðîöåññå ìàññîâîãî ñîõðàíåíèÿ,
		# äî ñàìîãî ñîõðàíåíèÿ.
		# ñîõðàíåíî áóäåò òî, ÷òî âûäåëåíî ê ñîõðàíåíèþ â ýòîé ñòðóêòóðå. íà êëèåíòå àêòèâèðîâàòü êíîïêó ñîõðàíåíèÿ òîëüêî ïîñëå ïðèõîäà ýòîé ñòðóêòóðû
		# ëþáîå èçìåíåíèå â ýòîé ñòðóêòóðà äîëæíî áëîêèðîâàòü ñîõðàíåíèå. åñëè èçìåíèëè òî îïÿòü ïðîâåðêà.
		
		# Åñëè àðòèñò óæå â áàçå, òî îí ïðåäëàãàåòñÿ êàê íåâûáðàííûé checked = False, ïðîâåðêà åãî àëüáîìîâ ïðîèñõîäèò âñåãäà
		# ïðîâåðî÷íàÿ èåðàðõèÿ âñåãäà åñëè åñòü àëüáîìû íà ñîõðàíåíèå
		
		# Åñëè àðòèñò íå â ÁÄ è íå âûäåëåí, òî îòìåíèòü âûáîðêó  åãî àëüáîìîì, äàæå åñëè îíè âûáðàíû
	...
	def artist_album_struct_client_server_sinc_categ(self,categ_key,artist_album_struct_serv,artist_album_struct_client,list_node_key,new_process_type,mode):
		# !!!Âíèìàíèå ôóíêöèÿ  âûçûâàåòñÿ ÒÎËÜÊÎ äëÿ îòäåëüíîãî âüþ.!!! èëè ÀÐÒÈÑÒ èëè ÀËÜÁÎÌ
		...
		# 1 Äëÿ ðåæèìà add ïðîâåðêà, ÷òî ýëåìåíòû íå ñóùåñòâóþò â áàçå.
		if mode == "add":
			db = sqlite3.connect(self.__model_instance.getMediaLibPlayProcessContext()['dbPath'])
			db.text_factory = str
			c = db.cursor()
		...
		# Äëÿ ðåæèìà äîáàâëåíèÿ, âñå ÷òî ñåëåêòåä + âñå ÷òî íå âûáðàíî - èãíîðèðóåòñÿ. ñâåðòûâàåòñÿ, íåâèäèìèòñÿ.
			...
			for a in artist_item_serv:
				...
				if a == 'checked' :
					...
					if mode == "add" and ( not artist_item_client['checked'] ):
						...
						# Ýòî ïîëå ñëóæèò èíäèêàòîðîì, ÷òî åãî íàäî ó÷åñòü ïðè àïäåéòå ÁÄ. ëèáî óäàëèòü, ëèáî äîáàâèòü
						artist_item_client['process_type'] = new_process_type
						if mode == "update":
							if artist_item_client[a] <> artist_item_serv[a]:
								
								if not artist_item_client[a]:   # çíà÷èòü óäàëèòü
						...
						if mode == 'add' and ( not album_item_client['checked'] ):
							...
							if 	mode == 'update':
								if album_item_client[b] <> album_item_serv[b]:
									...
									if not album_item_client[b]:   # çíà÷èòü óäàëèòü
	...
	def artist_album_struct_client_server_sinc(self,artist_album_struct_serv,artist_album_struct_client,list_node_key,new_process_type):
		...
		for artist_item_serv in artist_album_struct_serv:
			...
			for album_item_serv in albumL_serv:
				...
				# ïðîïóñê ïîçèöèé óæå íàõîä. â áä èëè íåâûáðàííûõ
	...
	def artist_album_maintain_check(self,commandD):	
		# ôàçà ïðîâåðêè äîëæíî ïðèéòè òèï ïðåäñòàâëåíèÿ, òèï ñîñòîÿíèÿ ïðîöåññà, íîìåð ñòðàíöèöû.
		# îáíî
		# Âíèìàíèå!!! Ýòà ôóíêöèÿ íå ïîëíîñòüþ óíèâåðñàëüíàÿ, ò.ê. â çàâèñèìîñòè îò ïðîöåññà îáðàáîòêè (íà÷àëüíîå ñîõðàíåíèå, èëè êàòåãîðèçàöèÿ) âûçûâàåìàÿ ôóíêöèÿ
		# ñèíõðîíèçàöèè artist_album_struct_client_server_sinc èëè åå àíàëîã ðàáîòàåò ïî ðàçíîìó, ÷òîáû âûçâàòü ïðàâèëüíóþ ôóíêöèþ åå íàäî âûçûâàòü â êîíòåêñòå ïðîöåññà
		# ýòîò êîíòåêñò ïåðåäàåòñÿ ïîêà ÷åðåç çíà÷åíèå ïàðàìåòðà commandD['artists_maintain'] 
		...
		if ('artistDataD' in commandD or 'album_NSA_DataD' in commandD) and 'tb_save' in commandD['action_mode']:
			...
			# Ïðîâåðèòü ëîãè÷åñêóþ öåëîñòíîñòü àëüáîìîâ è àðòèñòîâ è âåðíóòü ïðåäëîæåíèå íà ñîõðàíåíèå
			# ê äàííûì ñòðóêòóð íàäî äîáàâèòü ñåññèîííûé êëþ÷ óíèêàëüíîñòè áóôåðà, êîòîðûé âåðíóâøèñü íàçàä äîëæåí áûòü ñâåðåí ñ ñåðâåðíûì åñëè îê. òî ìîæíî ñîõðàíÿòü
			# íà êëèåíòå ëþáûå èçìåíåíèÿ ñôîðìèðîâàííîé íèæå ñòðóêòóðû îòìåíÿþò ñîõðàíåíèå ïîêà ñâåðêà íå ïðîéäåò.
			
			# Íèæå èäåò âûáîð ñîîòâåòñòâóþùåé ïðîöåññó ôóíêöèè ñèíõðîíèçàöèè
			...
			# ôîðìèðóåì áóôåð ïðîöåññà ìàññîâîé îáðàáîòêè, ñîõðàíèâ â íåì ïðèøåäøóþ ñ êëèåíòà ðåïëèêó ïî êëþ÷ó artist_dataD_list_client_copy_init_edited
	...
	def album_NSA_view_save(self,album_view_dataD,albumD,db):	
		...
		for album_item_serv in album_view_dataD:
			...
			# Ñâÿçè ñîõðàíÿþòñÿ â ëþáîì ñëó÷àå ïî ëþáûì àðòèñòàì. ãëàâíîå ÷òîáû ñóùåñòâîâàë àëüáîì
			...
			for artist_item_serv in artistL_serv:
				...
				# Ëîãèêà ñîõðàíåíèÿ ñëåäóþùàÿ, òê. àëüáîì ëîãè÷åñêè íå ìîæåò áûòü áåç àðòèñòà àðòèñòîâ, òî âíà÷àëå íàäî ñîçäàòü àðòèñòà
				# Çàòåì îäèí ðàç ñîçäàåòñÿ àëüáîì åñëè åãî íàäî ñîçäàâàòü
				...
				if not artist_item_serv['is_in_db']:	
					...
					# Åñëè îøèáêà ñîõðàíåíèÿ òî ñìåíèòü öâåò íà îøèáî÷íûé êàê äëÿ ñïèñêà
		...
		for artist_item_serv in artist_view_dataD:
			...
			for album_item_serv in albumL_serv:
				...
				if not album_item_serv['checked']:
					continue
				# Åñëè àðòèñò íå â áàçå ì ÷åêíóò	
				if album_item_serv['process_type'] == 'tb_save':
					# òóò ñîõðàíèòü àËüáîì
	...
	def artist_album_maintain_mass(self,commandD):	
		# Ôóíêöèÿ îáåðòêà äëÿ ñîõðàíåíèÿ äàííûõ â ÁÄ (artist_view_save)
		...
		if ('artistDataD' in commandD or 'album_NSA_DataD' in commandD) and 'save' in commandD['action_mode']:
			# Òóò íóæíà ñåðüåçíàÿ ïðîâåðêà, ÷òî ïîñëàííûå øàã íàçàä äàííûå íå èçìåíèëèñü
			# åñëè ïðîâåðêà ïðîøëà òî ñîõðàíÿòü
			# äàííûå îò êëèåíòà ïåðåäàâàòü íå íàäî
	...
	def artist_album_categ_maintain_mass(self,commandD):	
		...
		if ('artistDataD' in commandD or 'album_NSA_DataD' in commandD) and 'save' in commandD['action_mode']:
			...
			artistD = {} # âçÿòü ìåòàäàííûå òóò èç áóôåðîâ
	...
	def get_images_4_cur_album_fromPL(self,commandD):	
		...
		# Íàéòè ñðö32 òåêóùåãî àëüáîìà album_crc32 = metaD_of_cur_pL[1989996026]['album_crc32']
	...
	def get_tracks_4_selected_folder(self,commandD):	
		# Ñáîð ìåòàäàííûõ äëÿ çàêëàäêè Tracks è óïàêîâêà â JSON ñòðóêòóðó
	...
	def play_album_from_folder(self,commandD):
		...
		# ïîëó÷èòü ìåòàäàííûå èç ñîõðàíåííîãî get_tracks_4_selected_folder ïî album_id
	...
	def get_picture(self,commandD):	
		# Çàãðóçêà êàðòèíêè ïî URL èç àäìèí çàêëàäêè
		...
		# Ïðîâåðèòü ÷òî êàðòèíêà äëÿ ÐÀÄÈÎ, ïðîâåðèòü åñòü ëè ïàïêà äëÿ íåå ïî óçëó èç øàáëîíà. åñëè íåò ñîçäàòü å???
	...
	def get_object_edit_data(self,commandD):	
		...
		if object_type == 'artist':
			...
			# Ðåôåðåíòíûå ñïèñêè ÷åðåç ôóíöèþ getAll_Related_to_main_Artist_fromDB:
			#1. Ñïèñîê ñî ññûëêîé èëè ññûëêàìè íà ãëàâíûõ (äëÿ playtogether èõ íåñêîëüêî)
			#2. Ñïèñîê ñî ññûëêàì îò ãëàâíîãî (äëÿ ãëàâíîãî)
			#3. Ñïèñîê âñåãî êëàñòåðà äëÿ íåãëàâíîã
			artists_from_main_relL = [] 
			artists_main_to_relL = [] 
			artists_all_relL = [] 
			artists_from_main_relLD = [] # äëÿ ãëàâíîãî ñïèñîê ïîä÷èíåííûõ
			artists_main_to_relLD = [] # ñïèñîê ãëàâíûõ àðòèñòîâ äëÿ íåãëàâíîãî èëè âèðòóàëüíûõ äëÿ íåãëàâíîãî
			artists_all_relLD = [] # ñïèñîê âñåãî êëàñòåðà äëÿ íåãëàâíîãî
			...
			# Ïîëó÷àåì äàííûå ïî âñåì àðòèñòàì îíè íóæíû ÷òîáû âîñòàíîâèòü èñõîäíûå äàííûå â ðåôåðåíòíûõ ñïèñêàõ
			artist_dbD_all=getArtistD_fromDB(dbPath,None,[],'wo_reflist')
			
			# Ïîëó÷àåì ðåôåðåíòíûå ñïèñêè ôðîì äëÿ ãëàâíîãî è òó äëÿ îáû÷íûõ
			...
			for rel_item in album_artist_metaD_db['artist_album_refLD']:
				...
				# Ýòî íàäî ìåíÿòü, çàáèðàòü àðòèñòîâ íàäî ïðîâåðÿÿ èõ ñâÿçü â ARTIST_ALBUM_REF
	...
	def maintain_single_artist_rel(self,commandD):
		# òî÷å÷íàÿ ôóíêöèÿ îáðàáðàáîòêè ïðîöåññà èçìåíåíèÿ Àëüáîìà èëè Àðòèñòà è âñåõ èõ ñâÿçåé
	...
	def maintain_single_artist_album(self,commandD):
		# òî÷å÷íàÿ ôóíêöèÿ îáðàáðàáîòêè ïðîöåññà èçìåíåíèÿ ñâÿçè Àëüáîìà èëè Àðòèñòà
		dbPath = self.__model_instance.getMediaLibPlayProcessContext()['dbPath']
	...
	def do_edit_artist(self,commandD):	
		...
		if 'artist' in commandD:
			...
			##################
			# Íàïèñàòü ôóíêöèþ áåðóùóþ òèï ñâÿçè äëÿ òåêóùåãî àðòèñòà è åãî ñâÿçàííûõ ñîñåäåé åå èñïîëüçîâàòü ïðè ïîêàçå ñòðàíèöû â äðîï äàóíå.
			...
			ref_art_newL = []
			# Åñëè íîâûé àðòèñò íà ââîä òî ó íåãî íåò ðåôåðåíñèâíûõ ñïèñêîâ ïîêà. çíà÷èò ïðîïóñê
			if len(artist_dbD) == 1:
	...
	def get_track_search_list_for_tag_id(self,commandD):	
		# âîçâðàùàåò ñïèñîê "ïîèñêîâîãî" òèïà äëÿ âûáðàííîãî tag_id
	...
	def check_and_add_missd_song(self,commandD):
		# Íåäîäåëàííàÿ ïîêà ôóíêöèÿ àâòîàìàòè÷åñêéî ïðîâåðê òýãà íà íàëè÷èå íîâîé âåðñèè ïåñíè
		...
		search_term = ''
		dbPath = self.__model_instance.getMediaLibPlayProcessContext()['dbPath'] 
		# Ïîëó÷èòü âñå òðýêè ýòîãî íàçâàíèÿ, íåâçèðàÿ íà òî, ÷òî îíè óæå ñóùåñòâóþò â íàçíà÷åíèè
	...
	def get_artist_search_list_for_tag_id(self,commandD):	
		...
		db = sqlite3.connect(self.__model_instance.getMediaLibPlayProcessContext()['dbPath'])		
		#çàìåíèòü ýòó ôóíêöèþ íà íîâóþ äëÿ àðòèñòà
		dbIdL = getDbIdL_viaTagId(tagId,db)
		#print 'get_tag_tracks dbl_d:',dbIdL
		# òóò áðàòü èç RepBuffer
		sD = getCurrentMetaData_fromDB_via_DbIdL(dbIdL,db)
			#print sD
			#print sD[sD.keys()[0]].keys()
		db.close()
	...
	def artist_tag_assign_update_check(self,commandD):	
		# ïðîâåðÿåò ñïèñîê "ïîèñêîâîãî" òèïà äëÿ âûáðàííîãî tag_id íà ïðåäìåò èçìåíåíèÿ òýãîíàçíà÷åíèÿ
		
		# Äëÿ ïðåäñòàâëåíèÿ íàäî ïîäãîòîâèòü ñëåäóþùèå ïàðàìåòðû 'change_list':'','change_action':''
		# 'change_list':'' - ñïèñîê èçìåíåííûõ ïîçèöèé. 'change_action' - óäàëèòü èëè äîáàâèòü
		# âüþ äîëæåí âîçâðàòèòü íîâûé ëèñò ñ ïîäêðàøååíûìè ïîçèöèÿìè. ïðîìåæóòî÷íî äîëæíåí áûòü ñîõðàíåò ñïèñîê, ïî íåìó ïîñëå íàæàòèÿ save áóäåò ñäåëàí ðåàëüíûé
		# àïäåéò
		
		# òóò æå äåëàòü ïðîâåðêó öåëîñòíîñòè. åñëè äåéñòâèå äîáàâèòü òî ñïèñîê íå äîëæåí ïåðåñåêàòüñÿ ñ ñóùåñòâóþùèì. åñëè óäàëèòü, òî ýëåìåíòû äîëæíû áûòü â 
		# òýãîíàçíà÷åíèè
		...
		# âðåìåííàÿ ïðîâåðêà ñîõðàíåíèÿ â ÁÄ
		...
		# åñëè 'action_mode' == "delete" òî äîñòàòî÷íî ïðîñòî ñîõðàíèòü äåëüòà ñïèñîê êëþ÷åé, åñëè "add" òî íàäî äîïîëíèòü ïîèñêîâûé áóôåð ìåòàäàííûìè òðýêîâ òýãà,
		# íà êîòîðûé áóäóò íàçíà÷àòüñÿ òðýêè
		...
		if 'action_mode' in commandD and 'selL'  in commandD:
			...
			if commandD['action_mode'] == 'add' and len(commandD['selL']) > 0:
					...
					# ñïèñîê êëþ÷åé èç ïîèñêîâîãî ñïèñêà êîòîðûå âûáðàíû íà íàçíà÷åíèå
			...
			# Ñîõðàíÿåì äåëüòà ñïèñîê ïî êîòîðîìó áóäåò â ïîñëåäñòâèè âîçìîæíî ïðîèçâåäåí àïäåéò
			if sD_editable <> {}:
				# òóò ïîòîì åùå ñäåëàòü ïðîâåðêó íà ñîîòâåòñòâèå ñïèñêà èçìåíåíèé è òåêóùåãî ïîèñêîâîãî áóôåðà sD
				# òàêæåäëÿ äåëüòà ñïèñêà ìîæíî õðàíèòü òàéìñòåìï ñîçäàíèÿ è åñëè íà âðåìÿ ñîõðàíåíèÿ ïðîøëî äîïóñòèì áîëüøå ìèíóòû, òî 
				# òî îòêëîíÿòü òàêóþ òðàíçàêöèþ
			...
			for a in tag2trackD:
				...
				for b in tag2trackD[a]:
					...
					try:
						...
						# ïðîâåðÿåì íà ðåãèñòðàöèþ â áàçå àðòèñòîâ
						...
						# Äàëåå èäåò îáðàáîòêà òîëüêî äëÿ çàðåãèñòðèðîâàííûõ àðòèñòîâ		
						if artist_key in artist_mainL:
							...
							# ïîëó÷èòü ðåôåðåíòíûé ãëàâíûé èíäåêñ àðòèñòà
...
class MediaLib_Application_RPC_server():
	...
	def __init__(self):
		...
		# òóò òàêæå íàïðÿìþó âûçûâàåì ðîäèòåëüñêèé êîíñòðóêòîð, ÷òîáû èìåòü âîçìîæíîñòü äîñòóïà ê ôóíêöèÿì ñòàðîé è íîâîé ðåàëèçàöèè.
		# çàòåì ñåðâåð ñòàðòîâàòü â îòäåëüíîì ìîäóëå èñïîëüçóÿ èíñòàíöèþ êîíòðîëëåðà êàê âíåøíèé îáúåêò
	...
	def runApplServer(self):
		...
		# ôóêíöèè ñòàðîãî êîíòðîëëåðà â ìîäåëè (äëÿ ñîâìåñòèìîñòè) îò íèõ íàäî èçáàâèòüñÿ è âñå ïåðåíåñòè â íîâûé êîíòðîëëåð
