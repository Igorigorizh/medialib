...
class Winamp(object):
	...
	IPC_ENQUEUEFILE = 100
	# add item to playlist in UNICODE
	...
	# gets track pos in list
	IPC_GET_TRACK_POS = 125
	# gets given index playlist file
	IPC_GETPLAYLISTFILE = 211
	# gets given index playlist file in UNICODE
	...
	BUTTON_COMMAND_FORWARD = 40148
	BUTTON_COMMAND_REWIND = 40144
	...
	def enqueueFileW(self, filePath):
		"""Enqueues a file in Winamp's playlist.

		filePath is the given file path to enqueue.
		"""
		
		cpyData = create_unicode_buffer(filePath)
		cds = self.COPYDATASTRUCT(c_ulong(self.IPC_ENQUEUEFILEW),
								 c_ulong(len(cpyData)*2),
								 cast(cpyData, c_void_p))
	...
	def getVersion(self):
		"""Gets Winamp's Version 		"""
		iVersionNumber = self.__sendUserMessage(0, self.IPC_VERSION)
	...
	def setPlayingTrackPosition(self,iPositionMsecs):
		"""Gets the position in milliseconds of the current playing track."""
		return self.__sendUserMessage(iPositionMsecs,106)	
	...
	def getPlaylistPosition(self):
		"""Gets the current track  position in the playlist."""
		return self.__sendUserMessage(0,self.IPC_GET_TRACK_POS)
	...
	def flashPlaylistBuffer(self):
		"""flashPlaylistBuffer"""
		return self.__sendUserMessage(self.FLUSH_BUFFER, 0) 
	def closeWinamp(self):
		"""Close Winamp"""
		return self.__sendCommandMessage(self.CLOSE_WINAMP, 0)
	...
	def getPlaylistFileW(self, position):
		"""Returns the filename of the current selected file in the playlist."""
		address = self.__sendUserMessage(position, self.IPC_GETPLAYLISTFILEW)
	...
	def getPlaylistFingerPrint(self):
		"""Calculates the unique key of the current playlist"""
		if self.getListLength() < 12:
			#ft_str = ''.join([self.getPlaylistFile(position) for position in range(self.getListLength())]) + str(self.getListLength())
			return zlib.crc32( ''.join([self.getPlaylistFile(position) for position in range(self.getListLength())]) + str(self.getListLength()))	
		else:
			step = int(self.getListLength()/5)
			return zlib.crc32(''.join([self.getPlaylistFile(pos) for pos in [0,step,int(self.getListLength()/2),self.getListLength()-1-step,self.getListLength()-1]])+str(self.getListLength()))
			
	
	def getPlaylistFilenames(self):
		"""Retrieves a list of the current playlist song filenames."""
		return [self.getPlaylistFileW(position) for position in range(self.getListLength())]
